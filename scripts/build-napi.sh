...
case "$(uname -s)-$(uname -m)" in
  Linux-x86_64) suffix=linux-x64-gnu; artifact="$repo_root/target/debug/libruflo_napi.so" ;;
  Darwin-x86_64) suffix=darwin-x64; artifact="$repo_root/target/debug/libruflo_napi.dylib" ;;
  Darwin-arm64) suffix=darwin-arm64; artifact="$repo_root/target/debug/libruflo_napi.dylib" ;;
