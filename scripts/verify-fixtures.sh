...
test -f "$fixtures_root/cli/security/overview.json"
test -f "$fixtures_root/cli/analyze/overview.json"
test -f "$fixtures_root/cli/daemon/overview.json"
test -f "$fixtures_root/cli/embeddings/overview.json"
...
if grep -rnE 'ghp_[A-Za-z0-9]{20,}|sk-[A-Za-z0-9]{20,}|AIza[0-9A-Za-z_-]{20,}|-----BEGIN [A-Z ]*PRIVATE KEY-----|xox[baprs]-[A-Za-z0-9-]{10,}' "$fixtures_root"; then
