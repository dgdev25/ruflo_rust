...
| Memory tools and hooks learning | MCP tools plus persisted state | `memory/store`, `memory/search`, `memory/list`, and `.swarm/memory.db` persistence | `/mnt/datadisk/dev/ruflo/v3/mcp/tools/memory-tools.ts`; `/mnt/datadisk/dev/ruflo/v3/@claude-flow/cli/src/mcp-tools/hooks-tools.ts` | Hooks persist ReasoningBank state into `.swarm/memory.db`; native persistence fixtures guard the compatible durable surface. |
...
| Windows hook parity smokes | Cross-platform hook behavior | generated hook commands and runtime execution on Windows, macOS, Linux | `tests/fixtures/consumers/platform-hooks/windows.json`; `scripts/release-smoke.ps1`; `docs/release/platform-support.md` | Run the documented smoke scripts on each target host before release; no GitHub Actions workflow is present. |
...
- The POSIX hook manifest in `plugin/hooks/hooks.json` explicitly documents that it remains Windows-incompatible, so the native rebuild carries separate checked-in POSIX and Windows hook fixtures and requires target-host smoke evidence before release.
