...
`mcp`, `config`, `migrate`, `version`, `completions`, `doctor`, and `progress`. This is not a
...
| Core state/transport | `memory`, `mcp`, `config`, `migrate`, `hooks`, `workflow` | `memory` now implements tested Rust CLI `store`, `retrieve`/`get`, `search`, and `list`/`ls` against the durable projection; its semantic CLI route, remaining subcommands, MCP parity, and source fixtures remain pending. `mcp start` exists. `config init` and `config get` use the native precedence layer, with V3 mutations/import/export/providers and fixtures pending. `migrate status` detects V2 config/memory/session artifacts; `migrate run --target config` has backup-gated V2 field transforms. Remaining migration targets and source fixtures are pending. Other families are pending |
