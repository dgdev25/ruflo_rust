...
| `memory` | `store`, `retrieve` (`get`), `search`, `list` (`ls`), `delete` (`rm`), and `stats` have tested initial native CLI implementations. Search is explicitly keyword-projection behavior until the production embedding route uses the existing AgentDB/RVF facade; `purge`, `configure`, `cleanup`, `compress`, `export`, `import`, `init`, `distill`, `backup`, `classify`, and `select-operator` remain pending, as do source fixtures |
