...
| --- | --- |
| `agent` | `spawn`, `list` (`ls`), `status`, `stop` (`kill`), and `metrics` have initial durable native implementations; `pool`, `health`, `logs`, `wasm-status`, `wasm-create`, `wasm-prompt`, `wasm-gallery`, and `publish` remain pending, as do differential V3 fixtures for all agent operations |
