...
| `swarm` | `init`, `start`, `status`, `stop`, and `scale` have initial durable native implementations. `start` uses the existing policy-gated native Codex scheduler without API keys; `coordinate`, `compress-message`, `pheromone`, and `join` remain pending, as do source fixtures for all subcommands |
