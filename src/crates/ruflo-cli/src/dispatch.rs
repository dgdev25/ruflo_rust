//! CLI command dispatch extracted from lib.rs.
use std::ffi::OsString;
use std::process::ExitCode;
use crate::command::{self, ParsedCommand};
use crate::{
    analyze, announcements, appliance, appliance_advanced, auth, autopilot, benchmark,
    benchmarks, claims, cleanup, completions, compressor, config_file, daemon,
    deployment, distillation, eject, embeddings, funnel, funnel_command, funnel_extra,
    flywheel_ledger, gaia_bench, graph_algo, guidance, harness_exec, hooks, hive_mind,
    init_wizard, issues, lifecycle, metaharness, neural, onnx_embeddings, output,
    performance, plugins, policy, prompt, process_cmd, prod_modules, repo_supervisor,
    providers, proxy, registry_api, route, security, services, settings, small_modules,
    sona, spend, swarm_exec, spinner, transfer_store, update_cmd, verify, version, workflow,
};
